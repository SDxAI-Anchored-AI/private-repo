...
export function ChatMessage(props: {
  ...
  onImagine: (messageText: string) => void;
}) {
  const {
    text: messageText,
  ...
  const isImaginableEnabled =
    messageText?.length > 5 && !messageText.startsWith('https://images.prodia.xyz/') && !(messageText.startsWith('/imagine') || messageText.startsWith('/img'));
          ...
          {!errorMessage &&
            parseBlocks(fromSystem, collapsedText).map((block, index) =>
              block.type === 'html' ? (
                ...
                <RenderBlob key={'blob-' + index} blobBlock={block} />
              ) : renderMarkdown ? (
